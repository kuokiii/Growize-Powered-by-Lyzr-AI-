import { Target, CheckSquare, AlertTriangle, Users, DollarSign, Clock, Zap, Sparkles } from "lucide-react"
...
export default function DecisionFrameworks({ growizeData }: { growizeData: GrowizeData }) {
  // If we have real AI insights, we could parse decision frameworks from them
  if (growizeData.aiInsights) {
    ...
    // Parse decision frameworks from the AI response
    const parseDecisionFrameworks = (response: string) => {
      const frameworkMatch = response.match(
        /\*\*Decision-Making Frameworks Suitable for Your Business:\*\*(.*?)(?=\*\*|$)/s,
      )
      return frameworkMatch ? frameworkMatch[1].trim() : ""
    }

    const frameworkContent = parseDecisionFrameworks(aiResponse)
      ...
      <div className="space-y-8">
        <div className="bg-gradient-to-r from-teal/10 to-purple-500/10 rounded-2xl p-6 border border-teal/20">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h3 className="text-lg font-bold text-charcoal">AI-Generated Decision Frameworks</h3>
              <p className="text-sm text-charcoal/60">Custom frameworks recommended by Lyzr AI</p>
            </div>
          </div>

          <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            <div className="prose prose-sm max-w-none">
              <div className="whitespace-pre-line text-charcoal/80 leading-relaxed">
                {frameworkContent || aiResponse}
              </div>
            </div>
          </div>
  ...
  // Fallback to mock decision frameworks
  const frameworks = [
    {
  ...
  const getColorClasses = (color: string) => {
    switch (color) {
      case "teal":
        return "bg-gradient-to-br from-teal to-teal/80 text-white border-teal/20"
      case "purple":
        return "bg-gradient-to-br from-purple-500 to-purple-600 text-white border-purple-200"
      case "orange":
        return "bg-gradient-to-br from-orange-500 to-orange-600 text-white border-orange-200"
      default:
        return "bg-gradient-to-br from-gray-500 to-gray-600 text-white border-gray-200"
    }
  }
    ...
    <div className="space-y-8">
      <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-6">
        ...
        <p className="text-yellow-700 text-sm">
          These are sample decision frameworks. The form will connect to Lyzr AI for personalized frameworks.
        </p>
      </div>
        ...
        {frameworks.map((framework, index) => (
          <div key={index} className="glass-effect rounded-2xl p-8 border border-white/20">
            <div className="flex items-start space-x-4 mb-6">
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center shadow-lg ${getColorClasses(framework.color)}`}
              >
                    ...
                    <div key={criterionIndex} className="flex items-start space-x-3">
                      ...
                      <div>
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          <span className="text-xs bg-teal/10 text-teal px-2 py-1 rounded border border-teal/20">
                            {criterion.weight}
                          </span>
                        </div>
              ...
              <div className="bg-gradient-to-r from-sage/10 to-teal/10 rounded-xl p-6 border border-sage/20">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-charcoal/70 text-sm">{framework.example.initiative}</span>
                  </div>
      ...
      <div className="bg-gradient-to-r from-teal/10 to-sage/10 rounded-2xl p-8 border border-teal/20">
